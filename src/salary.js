...
function calculatePercentages(fullSalary) {
  const { discountINSS, discountIRPF } = calculateSalaryFrom(fullSalary);

  const percDiscountINSS = round((discountINSS / fullSalary) * 100); 
  const percDiscountIRPF = round((discountIRPF / fullSalary) * 100);  
  const percNetSalary = round(100 - (percDiscountINSS + percDiscountIRPF)); 
...
export { calculateSalaryFrom, calculatePercentages };
